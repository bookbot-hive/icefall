#!/usr/bin/env bash

# fix segmentation fault reported in https://github.com/k2-fsa/icefall/issues/674
export PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python

set -eou pipefail

nj=16
stage=-1
stop_stage=100

vocab_sizes=(
  1000
  # 500
)


phonemize=true
lang=id
subset=("train_refined" "dev" "test")

if [ $phonemize = true ] ; then
  for i in "${!subset[@]}"; do
    subset[$i]="${subset[$i]}.phoneme"
  done
fi

subsets=""
for i in "${!subset[@]}"; do
  subsets="$subsets ${subset[$i]}"
done

# Split L subset to this number of pieces
# This is to avoid OOM during feature extraction.
num_splits=1000

dl_dir=$PWD/download
splits_dir=$PWD/splits_dir

. shared/parse_options.sh || exit 1

# All files generated by this script are saved in "data".
# You can safely remove "data" and rerun this script to regenerate it.
mkdir -p data

log() {
  # This function is from espnet
  local fname=${BASH_SOURCE[1]##*/}
  echo -e "$(date '+%Y-%m-%d %H:%M:%S') (${fname}:${BASH_LINENO[0]}:${FUNCNAME[1]}) $*"
}

log "dl_dir: $dl_dir"


if [ $stage -le 0 ] && [ $stop_stage -ge 0 ]; then
  log "Stage 0: Download data"

  if [ ! -d $dl_dir/gigaspeech2 ]; then
    huggingface-cli download speechcolab/gigaspeech2 --include data/id/* --repo-type dataset --local-dir download/gigaspeech2
  fi

  if [ ! -d $dl_dir/common-voice-13_0-id ]; then
    lhotse download bookbot-huggingface bookbot/common-voice-13_0-id $dl_dir phonemes_ipa " "
  fi

  if [ ! -d $dl_dir/fleurs-id ]; then
    lhotse download bookbot-huggingface bookbot/fleurs-id $dl_dir phonemes_ipa " "
  fi

  if [ ! -d $dl_dir/bookbot_id_v4 ]; then
    lhotse download bookbot-huggingface bookbot/bookbot_id_v4 $dl_dir phonemes_ipa " "
  fi

  if [ ! -d $dl_dir/musan ]; then
    lhotse download musan $dl_dir
  fi

  if [ ! -d $dl_dir/audio_splits ]; then
    lhotse download hallway $dl_dir
  fi
fi

if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  log "Stage 1: Prepare manifests"
  mkdir -p data/manifests

  if [ ! -e data/manifests/.gigaspeech2.done ]; then
    # convert text to phonemes
    if [ $phonemize = true ] ; then
      ./local/phonemize_gigaspeech2_transcripts.py --download-dir $dl_dir/gigaspeech2
    fi
    lhotse prepare gigaspeech2 -l $lang -j $nj --phonemize $phonemize $dl_dir/gigaspeech2 data/manifests
    touch data/manifests/.gigaspeech2.done
  fi

  lhotse prepare bookbot-huggingface $dl_dir/common-voice-13_0-id data/manifests
  lhotse prepare bookbot-huggingface $dl_dir/fleurs-id data/manifests
  lhotse prepare bookbot-huggingface $dl_dir/bookbot_id_v4 data/manifests
  lhotse prepare musan $dl_dir/musan data/manifests
  lhotse prepare hallway $dl_dir/audio_splits data/manifests
fi

if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  log "Stage 2: Preprocess GigaSpeech manifest"
  if [ ! -f data/fbank/.gigaspeech_preprocess.done ]; then
    ./local/preprocess_gigaspeech2.py --lang $lang --dataset "$subsets"
    touch data/fbank/.gigaspeech_preprocess.done
  fi
fi

if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  log "Stage 3: Compute fbanks"
  mkdir -p data/fbank

  ./local/compute_fbank_gigaspeech2.py
  ./local/compute_fbank_commonvoice.py
  ./local/compute_fbank_fleurs.py
  ./local/compute_fbank_bookbot.py
  ./local/compute_fbank_musan.py
  ./local/compute_fbank_hallway.py
fi

if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
  log "Stage 4: Split train set into ${num_splits} pieces"
  split_dir=data/fbank/gigaspeech2_train_split_${num_splits}
  if [ ! -f $split_dir/.gigaspeech_train_split.done ]; then
    lhotse split $num_splits ./data/fbank/gigaspeech2_cuts_train_refined_raw.jsonl.gz $split_dir
    touch $split_dir/.gigaspeech_train_split.done
  fi
fi

if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
  log "Stage 5: Compute features for train set"
  # Note: The script supports --start and --stop options.
  # You can use several machines to compute the features in parallel.
  if [ ! -f data/fbank/.gigaspeech_train.done ]; then
    ./local/compute_fbank_gigaspeech2_splits.py \
      --dataset train_refined \
      --num-workers $nj \
      --batch-duration 600 \
      --num-splits $num_splits
    touch data/fbank/.gigaspeech_train.done
  fi
fi

if [ $stage -le 6 ] && [ $stop_stage -ge 6 ]; then
  log "Stage 6: Combine features for train set"
  if [ ! -f data/fbank/gigaspeech2_cuts_train.jsonl.gz ]; then
    pieces=$(find data/fbank/gigaspeech2_train_split_${num_splits} -name "gigaspeech2_cuts_train_refined.*.jsonl.gz")
    lhotse combine $pieces data/fbank/gigaspeech2_cuts_train_refined.jsonl.gz
  fi
fi

if [ $stage -le 7 ] && [ $stop_stage -ge 7 ]; then
  log "Stage 7: Prepare phone based lang"
  lang_dir=data/lang_phone
  mkdir -p $lang_dir

  ./local/prepare_lexicon.py \
   --manifests-dir data/manifests \
   --lang-dir $lang_dir

  if [ ! -f $lang_dir/L_disambig.pt ]; then
    ./local/prepare_lang.py --lang-dir $lang_dir
  fi
fi